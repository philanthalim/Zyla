...
const SearchResults = () => {
  ...
  const shop = location.state.query;
  const getResults = async () => {
    const res = await Axios.get(`https://zyla-app.herokuapp.com/api/reviews/`);
    const resultsList = res.data.filter((item) =>
      item.shop.toLowerCase().includes(shop.toLowerCase())
    );
    //setResultsList(res.data.filter((item) => item.shop === shop));
    //const arr = Array.from(new Set(resultsList.filter((item) => item.shop)));
    //console.log(arr, "arr");
    let newResultsList = [];
    let uniqueObject = {};
    for (let i in resultsList) {
      let shopName = resultsList[i].shop;
      uniqueObject[shopName] = resultsList[i];
    }

    for (let i in uniqueObject) {
      newResultsList.push(uniqueObject[i]);
    }
    ...
    <div>
      <div className="search-header-container">
        ...
        <h3 style={{ position: "absolute", color: "white" }}>
          {resultsList.length} Search Results for '{shop}'
        </h3>
      </div>
      {resultsList.map((shop) => (
        <Link
          to={{
            pathname: `/view-shop/${shop.shop
              .replace(/\s+/g, "-")
              .toLowerCase()}`,
